...
- **전이 화면을 설정으로** — `transition_screens: {대상 상태: [필드…]}`. 실 Jira 는 화면을 **어느
  전이에나** 붙인다(예: Reopen 에 담당자·코멘트 강제). 지금까지는 done 범주로 갈 때만 화면이
  붙게 박혀 있어, 되돌리는 전이에 입력을 요구하는 워크플로를 흉내 낼 수 없었다.
  필드 뒤에 `?` 를 붙이면 선택 입력. 설정을 비우면 예전과 같다(done 범주 기본 화면).
  `hasScreen` 도 실제 화면 유무를 따라간다.
- **주입된 프로젝트에도 이슈 생성** — `POST /rest/api/2/issue` 가 config 프로젝트만 받아,
  자기 world 를 주입한 소비자는 그 프로젝트에 이슈를 만들 수 없었다(`Unknown project`).
  키 번호도 **프로젝트마다 따로** 흐른다(안 그러면 이미 있는 키를 다시 내준다).
- **생성 시 우선순위·Epic Name 반영** + 하위 이슈는 부모의 모듈/컴포넌트를 물려받는다
  (안 그러면 새 Sub-Task 가 부모와 다른 모듈로 잡혀 모든 롤업에서 엉뚱한 칸에 들어간다).
